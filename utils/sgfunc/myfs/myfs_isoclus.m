function [surfs, lut] = myfs_isoclus(surfs, labels, cfg)
% [surfs, lut] = MYFS_ISOCLUS(surfs, labels, cots, cfg)
%
...
for s = 1:2
  ...
  for i = 1:max(idx)
    ...
    [~,c] = tricontourfast(surfs.(cfg.basesurf){s}, smoothclus, 0.5, false);
