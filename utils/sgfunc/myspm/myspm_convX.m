...
if ~iscell(job.onset_sec)
  job.onset_sec = {job.onset_sec};
end
if ~iscell(job.dur_sec)
  job.dur_sec = {job.dur_sec};
end
...
for irun = 1:num_runs
  ...
  SPM.Sess(irun).U = struct(...
    'name',{{job.regname}},...
    'ons',job.onset_sec{irun},'dur',job.dur_sec,...
  ...
  xX{irun} = X{irun}(...
    (0:(SPM.nscan(irun) - 1))*job.fMRI_T + job.fMRI_T0 + 32,:);
