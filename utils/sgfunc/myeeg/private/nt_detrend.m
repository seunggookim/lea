function [y,w,r]=nt_detrend(x,order,w,basis,thresh,niter,wsize)
...
% (1) divide into windows, (2) detrend each, (3) stitch together, 
% (4) estimate w
...
if 0
    ...
    WSIZE=200;
    [y1,w1]=nt_detrend(x,3,[]);
    [y2,w2]=nt_detrend(x,3,[],[],[],[],WSIZE);
    figure(1); clf; 
    plot([x,y1,y2]); legend('before', 'after (win=whole)','after (win=200)');
end
if 0
    % weights
