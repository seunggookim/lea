 function [w,y,h]=nt_outliers(x,w,thresh,niter)
...
h= figure('position',[figpos(1:2) 580 749]);
subplot(3,1,1:2)
scores  = []; 
iterlegends = {};
fprintf('First iteration will take a bit longer...\n')
for iIter=1:niter
    %{ 
    ...
    nPatterns=size(ww,1);
    fprintf('Iter%d: Found %d unique weight sets.\n',iIter, nPatterns)
...
%     disp([num2str(iIter), ', score: ',num2str(sum(score))]);
    fprintf('\nsum diff=%.4f\n',sum(score));
    scores = [scores; score];
    iterlegends = [iterlegends, ['Iter',num2str(iIter)]];
    plot(score,'o-','linewidth',2); hold on; 
    legend(iterlegends,'location','best','box','off')
    xlabel('Channel index')
    ylabel('d avg. over time')
    set(gca,'fontsize',13); grid on
    drawnow    
end % iterations
subplot(3,1,3)
plot(sum(scores,2),'-o','linewidth',2)
xlabel('Iteration')
ylabel('avg d summed over chans')
grid on
set(gca,'fontsize',13)
