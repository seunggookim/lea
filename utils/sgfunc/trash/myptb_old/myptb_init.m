...
Job = defaultjob(struct(Machine='mpieaeeg1', Debug=true, AudioMode=1, AudioSrateHz=44100, AudioReqLatencyClass=3, ...
  AudioNumChannels=2, TextFont='Monospace', TextSize=18, TextColor=[0 0 0], FillRect=round(255*0.8*[1 1 1]), ...
  Vspace=2), Job, mfilename);
...
Idx = find(contains(lower({Dev.DeviceName}), lower(Job.AudioDevice)));
if numel(Idx)==1
  logthis('Audio output device "%s" will be used...\n', Dev(Idx).DeviceName)
  Job.PtrAud = PsychPortAudio('Open', Dev(Idx).DeviceIndex, Job.AudioMode, Job.AudioReqLatencyClass, ...
    Job.AudioSrateHz, Job.AudioNumChannels);
else
