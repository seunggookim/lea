function [stats,st] = SStat_mass_CoxStratPH(X,z,d,t,Y,maskvtx,prs,e)
...
nv = size(Y,2);
[m,p] = size(X);
if (length(z)~=m) || (length(d)~=m) || (length(t)~=m) || (size(Y,1)~=m)
    error(['The design matrix X, strata vector z, censorship status vector d'...
        ' ,time vector t and data matrix Y must all have the same number of rows.']);
end;
...
parfor j=1:nv
    ...
    while (gnorm>e) && (it<=nit)
        ...
        He = zeros(p,p);
        for k=1:length(uz)
            gr = gr + SStat_Gradient(XD(z==uz(k),:),Bhat,ft_ix(k,1:nft(k)),nties(k,1:nft(k)));
            He = He + SStat_Hessian(XD(z==uz(k),:),Bhat,ft_ix(k,1:nft(k)),nties(k,1:nft(k)));
        end;
        if (cond(He) < 1e+10)
            invHe = He\eye(p);
        else
    ...
    llh = 0;
    for k=1:length(uz)
        llh = llh + SStat_Likelihood(XD(z==uz(k),:),Bhat,ft_ix(k,1:nft(k)),nties(k,1:nft(k)));
    end;
