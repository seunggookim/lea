function [H, cfg, base, data] = slicein(base, data, cfg)
%SLICEIN create a single slice in a given axes (not the whole figure)
%
% [H, cfg] = slicein(base, data, cfg)
%
% [INPUT]
...
cfg = defaultcfg(struct(method='linear', basemethod='linear', contoursmoothing=1, xyz='axi1', axes=gca, ...
  ncontourlevels=1), ...
  cfg, mfilename, false);
...
assert(nslices==1, 'use SLICEIN for a SINGLE SLICE!')
