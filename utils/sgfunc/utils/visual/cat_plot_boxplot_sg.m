function [out,s] = cat_plot_boxplot_sg(data,opt)
...
%                                   % 3- scattered transparent circles 4- unscattered transparent circles
...
% _________________________________________________________________________
% $Id: cat_plot_boxplot.m 1341 2018-07-27 13:14:51Z dahnke $
...
def.fill        = 1;
try
  def.groupcolor = brewermap(numel(data),'Dark2');
catch ME
...
for i=1:qn
    ...
    if opt.box
      ...
      plot(cap_x(:,[i i+qn]), cap_y(:,[i i+qn]),'Color',[0.5 0.5 0.5],'linewidth',1);
      plot(whisker_x(:,[i i+qn]), whisker_y(:,[i i+qn]),'Color',[0.5 0.5 0.5],'linewidth',1);
    ...
    elseif opt.showdata == 2
      ...
      %---SG:
    elseif opt.showdata == 3 % scattered transparent circles
      if opt.vertical
%         plot(i*ones(1,length(data{i}))+(rand(1,length(data{i}))-0.5)/3, data{i}(:), 'o', 'MarkerFaceColor',0.25*opt.groupcolor(i,:), 'MarkerFaceColor',[1 1 1]);
        scatter(i*ones(1,length(data{i}))+(rand(1,length(data{i}))-0.5)/3, data{i}(:), 'o', 'MarkerFaceColor',0.25*opt.groupcolor(i,:), 'MarkerEdgeColor','w','MarkerFaceAlpha',0.3,'MarkerEdgeAlpha',0);
      else
%         plot(data{i}(:), i*ones(1,length(data{i}))+(rand(1,length(data{i}))-0.5)/3, 'o', 'Color',0.25*opt.groupcolor(i,:));
        scatter(data{i}(:), i*ones(1,length(data{i}))+(rand(1,length(data{i}))-0.5)/3, 'o', 'MarkerFaceColor',0.25*opt.groupcolor(i,:), 'MarkerEdgeColor','w','MarkerFaceAlpha',0.3,'MarkerEdgeAlpha',0);
      end
    elseif opt.showdata == 4 % unscattered transparent circles
      if opt.vertical
        scatter(i*ones(1,length(data{i})), data{i}(:), 'o', 'MarkerFaceColor',0.25*opt.groupcolor(i,:), 'MarkerEdgeColor','w','MarkerFaceAlpha',0.3,'MarkerEdgeAlpha',0);
      else
        scatter(data{i}(:), i*ones(1,length(data{i})), 'o', 'MarkerFaceColor',0.25*opt.groupcolor(i,:), 'MarkerEdgeColor','w','MarkerFaceAlpha',0.3,'MarkerEdgeAlpha',0);
      end
    elseif opt.showdata == 5 % scatter dots
      if opt.vertical
        plot(i*ones(1,length(data{i}))+(rand(1,length(data{i}))-0.5)/3,data{i}(:),'.','Color',0.25*opt.groupcolor(i,:));
      else
        plot(data{i}(:), i*ones(1,length(data{i}))+(rand(1,length(data{i}))-0.5)/3,'.','Color',0.25*opt.groupcolor(i,:));
      end     
    %---SG:
    end
...
%   $Revision: 1341 $  $Date: 2018-07-27 15:14:51 +0200 (Fr, 27 Jul 2018) $
