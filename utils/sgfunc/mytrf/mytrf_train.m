function model = mytrf_train(stim,resp,fs,Dir,tmin,tmax,lambda,varargin)
% model = mytrf_train(stim,resp,fs,Dir,tmin,tmax,lambda,varargin)
%
% THIS IS A MODIFICATION OF MTRFTRAIN SO THAT IT ALLOWS CHANNEL-SPECIFIC
% LAMBDAS EFFICIENTLY. (dr.seunggoo.kim@gmail.com; 2024-03-08)
%
...
I = regmat(nvar, arg.method);
w = zeros(nvar,yvar);
for thisLambda = unique(lambda)
  isYvar = lambda==thisLambda;
  % Fit linear model
  switch arg.type
    case 'multi'
      w(:,isYvar) = (Cxx + I*thisLambda/delta)\Cxy(:,isYvar)/delta; % (#xvar x #lag) x #chan
    case 'single'
      error('Now work on this')
%       w = zeros(nvar,nlag,yvar);
%       for i = 1:nlag
%         w(:,i,:) = (Cxx(:,:,i) + I*thisLambda/delta)\Cxy(:,:,i)/delta;
%       end
  end
end
...
elseif ~isnumeric(lambda) %|| ~isscalar(lambda) || lambda < 0
    error('LAMBDA argument must be numeric.')
end
