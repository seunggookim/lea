...
%%---SG[start]
% The current layout left out frontal area a lot, so I tilted 3-D position
% of channels so that they could be projected more anteriorly
if strcmp(ft_senstype(sens), 'yokogawa160')
 sens.chanpos = ft_warp_apply(rotate([-20 0 0]), sens.chanpos, 'homogenous');
end
%%---SG[end]
...
hold on
for i=1:157
  scatter(pos(i,1),pos(i,2),pos(i,3),'ro')
  text(pos(i,1),pos(i,2),pos(i,3),label{i})
end
...
hold on
for i=1:157
  scatter(prj(i,1),prj(i,2),'ro')
  text(prj(i,1),prj(i,2),label{i})
end
axis([-1 1 -1 1])
figure
  else
