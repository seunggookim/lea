function [stat, cfg] = musencfmri_clusterstatminp(job, statobs, statrnd)
%
% job
%  .BetasInfo
%  .tail
...
%
% (cc) 2021, sgKIM, solleo@gmail.com

job.dim = job.BetasInfo.InfoVTC.DimVTC;
job.inside = job.BetasInfo.InfoVTC.voxVTC;

[nvox, nrnd] = size(statrnd);
posdistributionminp = ones(1,nrnd);
negdistributionminp = ones(1,nrnd);

posobsminp = nan(nvox,1);
negobsminp = nan(nvox,1);
...
for icdt = 1:numel(job.clusteralphas)
  for icon = 1:numel(job.clusterconns)
    cfg = struct('tail',job.tail, 'dim',job.dim, 'inside',job.inside, ...
      'orderedstats','no', 'multivariate','no', 'minnbchan',0, 'wcm_weight',1,...
      'clusterstatistic','maxsum', 'clusterthreshold','nonparametric_common',...
      'clusteralpha', job.clusteralphas(icdt), 'clustercritval',[], ...
      'clustertail',job.tail, 'connectivity', job.clusterconns(icon), ...
    ...
    if isfield(stat1,'posdistribution')
      ...
      posdistributionminp = min([pospvaldistribution; posdistributionminp],[],1);
      
      % also keep the minimal observed p-values:
      posobsminp = min([stat1.prob, posobsminp],[],2);
      % or keep the cluster p-values?
      stats = [stats struct('posclusters',stat1.posclusters, ...
      'posclusterslabelmat',stat1.posclusterslabelmat)];
      
      if job.tail ~= 1
        error('now it is time to implement two-sided tests!')
      end
    end
  end
end
cfg.connectivity = job.clusterconns;
cfg.clusteralpha = job.clusteralphas;

stat = struct('posdistributionminp', posdistributionminp, ...
  'posobsminp', posobsminp, 'prob', nan(nvox,1));
% for each voxel?
for ivox = 1:nvox
  stat.prob(ivox) = (sum(posdistributionminp <= posobsminp(ivox))+1) / (nrnd+1);
end
...
% CLUSTER-BASED PERMUTATION TEST (ONLY FOR 3-D DATA)
% (CC) 2021, sgKIM.
...
%     tmp = reshape(postailobs, [cfg.dim 1]);
  ...
  posclusobs = findcluster3d(tmp, cfg.dim);
...
%     tmp = reshape(negtailobs, [cfg.dim 1]);
  ...
  negclusobs = findcluster3d(tmp, cfg.dim);
...
fprintf('computing clusters for the thresholded test statistic computed from the randomized design\n')
for i = 1:Nrand
...
%       tmp = reshape(postailrnd(:,i), [cfg.dim 1]);
    ...
    posclusrnd = findcluster3d(tmp, cfg.dim);
    if spacereshapeable
...
%       tmp = reshape(negtailrnd(:,i), [cfg.dim 1]);
    ...
    negclusrnd = findcluster3d(tmp, cfg.dim);
    if spacereshapeable
...
function [clus,num] = findcluster3d(onoff, dim)

BW = reshape(onoff, dim);
[L,num] = spm_bwlabel(BW, 18); % 18-connectivity (voxels sharing edges)
clus = L(:);
