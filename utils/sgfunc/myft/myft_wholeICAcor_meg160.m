function myft_wholeICAcor_meg160(fn_data,CFG)
[p1,f1,e1]=myfileparts(fn_data);
if ~exist('CFG','var')
  CFG=[];
end
if ~isfield(CFG,'bpfreq')
  CFG.bpfreq = [1 60]; % online filtering was bandpass=[1-200] notch=[60]
end
...
if ~exist(fn_ds,'file')
  ...
  % downsampling
  cfg=[];
  cfg.resamplefs = 200; % MEG160: from 1000 to 200 Hz
  ...
  for k=1:2 % k=1 EOG, k=2 ECG
    ...
    if prod(~isnan(idx_artcomp))
      ...
      for j=1:numel(idx_artcomp)
        ...
        axespos(ax,6);
        ft_topoplotIC(struct('component',c,'layout','yokogawa160_helmet.mat',...
...
cfg=[];
cfg.component=c;
cfg.layout='yokogawa160_helmet.mat';
cfg.comment='no';
cfg.markersymbol='.';
ft_topoplotIC(cfg, comp)
colormap(gca,flipud(brewermap(256,'spectral')))
