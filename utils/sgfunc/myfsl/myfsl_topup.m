...
%                      directions
% .fnames_to_unwarp  : a cell array of files to unwarp
...
%   'fnames_to_unwarp','myRawEPI_+j_250volumes.nii') )
%
%
...
%   'fnames_to_unwarp','myRawGEEPI_+j_250volumes.nii') ) ??
%
...
[p1,f1,e1] = myfileparts(Job.fnames_pe_pair{1});
dn_topup = [p1,'/',f1,'.topup'];
if not(isfolder(dn_topup)), mkdir(dn_topup); end
...
fn_data = [dn_topup,'/',prefix_out,'_data.nii'];
...
fn_acq = [dn_topup,'/',prefix_out,'_acq.txt'];
Job.fn_acq = fn_acq;
...
fn_field = [dn_topup,'/',prefix_out,'_field.nii'];
fn_unwarped = [dn_topup,'/',prefix_out,'_unwarped.nii'];
if ~exist(fn_field,'file')
  mysystem(['topup --imain=',fn_data,' --datain=',fn_acq,' --out=',p1,'/',prefix_out,' --fout=',fn_field,...
    ' --iout=',fn_unwarped,' -v --fwhm=8'])
  logthis('Unwarping field estimated: ')
  ls(fn_field)
end

%% APPLY deformation
if not(iscell(Job.fnames_to_unwarp))
  Job.fnames_to_unwarp = {fnames_to_unwarp};
end
for jFile = 1:numel(Job.fnames_to_unwarp)
  fn_data = Job.fnames_to_unwarp{jFile};
  [p1,f1,~] = myfileparts(fn_data);
  fn_unwarped = [p1,'/',f1,'_unwarped.nii'];
  if not(isfile(fn_unwarped))
    mysystem(['applytopup --imain=',fn_data,' --datain=',fn_acq,' --inindex=1 --topup=',[p1,'/',prefix_out],...
      ' --out=',fn_unwarped,' --method=jac']);
    assert(isfile(fn_unwarped))
    logthis('File unwarped: ')
    ls(fn_unwarped)
  end

  % copy json file with the same name
  fn_json = strrep(fn_data,'.nii','.json');
  fn_unwarped_json = strrep(fn_json,'.json','_unwarped.json');
  if isfile(fn_json) && not(isfile(fn_unwarped_json))
  ...
  if isfield(Job,'fname_t1w')
    [p2,f2,~] = myfileparts(fn_unwarped);
    fn_gif = [p2,'/',f2,'.gif'];
    if not(isfile(fn_gif))
      fn_t1w_coreg = myfsl_prept1wcoreg(Job.fname_t1w, fn_unwarped);
      compare_unwarped(fn_data, fn_unwarped, fn_t1w_coreg)
    end
  end
end
