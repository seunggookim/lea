function [stat, cfg] = clusterstat_par(cfg, statrnd, statobs)

% CLUSTERSTAT_PAR computers cluster statistic for multidimensional
% channel-freq-time or volumetric source data ... in PARALLEL
%
% See also TFCESTAT_PAR, FINDCLUSTER_NNBR
% ***CANNOT RUN MULTIVARIATE STATS!***
...
if isfield(cfg,'connectivity')
  connmat = full(cfg.connectivity);
else
  connmat = false;
end
...
switch cfg.clusterthreshold
  case 'parametric'
    if isempty(cfg.clustercritval)
      error('with parametric cluster thresholding cfg.clustercritval needs to be defined');
    end
  ...
  case 'nonparametric_individual'
    if isempty(cfg.clusteralpha)
      error('with nonparametric_indivdual cluster thresholding cfg.clusteralpha needs to be defined');
    end
  ...
  case 'nonparametric_common'
    if isempty(cfg.clusteralpha)
      error('with nonparametric_common cluster thresholding cfg.clusteralpha needs to be defined');
    end
...
if needpos
  ...
  posclusobs = findcluster_nnbr(tmp, connmat, 6, cfg.minnbchan);
...
if needneg
  ...
  negclusobs = findcluster_nnbr(tmp, connmat, 6, cfg.minnbchan);
...
%c = parcluster('local');
%delete(gcp('nocreate'));
%g = parpool(c, c.NumWorkers);  % by default use MAXIMAL # of workers on the system
parfor i = 1:Nrand
  [posdistribution(i),negdistribution(i)] = findrandcluster(...
  i,Nrand,spacereshapeable,needpos,needneg,cfg,postailrnd(:,i),negtailrnd(:,i),statrnd(:,i),connmat);
end % for 1:Nrand
%delete(g);
%ft_progress('close');
...
function [posdistribution_i,negdistribution_i] = findrandcluster(...
  i,Nrand,spacereshapeable,needpos,needneg,cfg,postailrnd_i,negtailrnd_i,statrnd_i,connmat)
posdistribution_i = nan;
negdistribution_i = nan;
fprintf('computing cluster stats for randomization (%i/%i)\n',i,Nrand);

if needpos
  if spacereshapeable
    tmp = zeros([1 cfg.dim]);
    tmp(cfg.inside) = postailrnd_i;
  else
    tmp = reshape(postailrnd_i, [cfg.dim 1]);
  end
  posclusrnd = findcluster_nnbr(tmp, connmat, 6, cfg.minnbchan);
  if spacereshapeable
  ...
  for j = 1:Nrndpos
    switch cfg.clusterstatistic
      ...
      case 'maxsum'
        stat(j) = sum(statrnd_i(posclusrnd==j));
      case 'wcm'
        if numel(postailcritval)==1
          posthr = postailcritval;
        elseif numel(postailcritval)==numel(posclusrnd)
  ...
  if strcmp(cfg.multivariate, 'yes') || strcmp(cfg.orderedstats, 'yes')
    ...
    if ~isempty(stat), posdistribution_i = max(stat); end
  end
end % needpos
if needneg
  if spacereshapeable
    tmp = zeros([1 cfg.dim]);
    tmp(cfg.inside) = negtailrnd_i;
  else
    tmp = reshape(negtailrnd_i, [cfg.dim 1]);
  end
  negclusrnd = findcluster_nnbr(tmp, connmat, 6, cfg.minnbchan);
  if spacereshapeable
  ...
  for j = 1:Nrndneg
    switch cfg.clusterstatistic
      ...
      case 'wcm'
        ...
        stat(j) = -sum((abs(statrnd_i(negclusrnd==j)-negthr)).^cfg.wcm_weight); % encoded as a negative value
      otherwise
        error('unknown clusterstatistic');
    end
  end % for 1:Nrndneg
  if strcmp(cfg.multivariate, 'yes') || strcmp(cfg.orderedstats, 'yes')
    ...
    %       end
    error('multivariate testing is not supported.')
  else
    % univariate -> remember the most extreme cluster, which is the most negative
    if ~isempty(stat), negdistribution_i = min(stat); end
  end
end % needneg
end
