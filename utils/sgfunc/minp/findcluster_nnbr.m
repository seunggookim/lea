function [cluster, numcluster] = findcluster_nnbr(onoff, spatdimneighbstructmat, nneighborhood, varargin)

% FINDCLUSTER returns all connected clusters for a three-dimensional N-connected
% neighborhood
%
% Use as
...
if ~(numel(siz)==1 && all(siz==1) && islogical(onoff))
  for j = 1:spatdimlength
    if numel(siz) <= 3 % if 2D or 3D data (without spatial dimension)
      ...
      [clus, num] = spm_bwlabel(double(onoff(seldims{:},j)), nneighborhood);
    else
